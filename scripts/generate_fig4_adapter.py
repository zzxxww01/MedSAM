#!/usr/bin/env python3
"""Fig.4 LG-Adapter Module Architecture
...
def main():
    ...
    ax.text(4.75, 9.0, "Local-Global Adapter Architecture", ha="center",
            fontsize=14, fontweight="bold", color="#2C3E50")

    # Input feature
    draw_box(ax, (2.5, 7.8), 4.5, 0.6,
             "Input: x in R^{Bx256x64x64}", "#34495E", 10)
    ...
    ax.text(9.3, 5.5, "Residual\nSkip", fontsize=8, color="#95A5A6",
    ...
    ax.text(2.2, 4.85, "Effective RF: 3x3", fontsize=7, ha="center",
            color="#E74C3C", fontstyle="italic")
    ax.text(7.3, 4.85, "Effective RF: 5x5", fontsize=7, ha="center",
    ...
    draw_box(ax, (2.75, 3.6), 4.0, 0.5,
             "Concat: [f1 || f2] -> Bx512x64x64", "#8E44AD", 9)

    # Pointwise Conv 1x1
    draw_arrow(ax, (4.75, 3.6), (4.75, 3.0), "#555")
    draw_box(ax, (2.75, 2.4), 4.0, 0.5,
             "Conv 1x1 (512->256) Cross-Scale Fusion", "#2C3E50", 9)
    ...
    draw_box(ax, (2.5, 0.0), 4.5, 0.5,
             "Output: y = x + f_out in R^{Bx256x64x64}", "#27AE60", 10)

    # Param count
    ax.text(4.75, -0.7, "Total params: ~136K (0.15% of model)", ha="center",

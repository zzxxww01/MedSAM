...
### 2026-02-14 进度更新（A0补齐 + A3R1运行中）

- A0 `Baseline` 同口径评估已完成（40例，CT_Abd）
  - 汇总: `work_dir/eval_metrics/A0_summary.json`
  - 指标: DSC=`0.940741`，HD95=`4.830503`，ASD=`0.537757`
- A3R1 修正实验已启动（长时任务，nohup）
  - 任务名: `MedSAM-FLARE22-A3R1-Balance-a0.5-b1.0-s70`
  - 运行参数: `alpha=0.5, beta=1.0, gamma=1.0, stage1_epochs=70, batch_size=1`
  - 日志: `work_dir/exp_logs/A3R1_train.log`
  - 状态: 已进入 `Epoch 0`，Rank0/1 双卡训练正常
- 当前关键数值（同口径）：
  - A0: DSC=`0.940741`, HD95=`4.830503`, ASD=`0.537757`
  - A1: DSC=`0.940596`, HD95=`4.790533`, ASD=`0.531697`
  - A2: DSC=`0.952554`, HD95=`3.368403`, ASD=`0.374899`
  - A3: DSC=`0.903470`, HD95=`7.922879`, ASD=`0.886811`
- 阶段结论：
  - Intra-CBL（A2）当前最优；
  - 完整 Balance（A3）存在退化；
  - R1 用于验证“降低 Inter 权重 + 延后切换”能否恢复性能。
- 下一阶段执行顺序：
  1. 完成 A3R1 训练与评估；
  2. 若 R1 仍低于 A2，启动 R2/R3 分离变量验证；
  3. 确定最优 Loss 配置后，再进入 EXP-005 Attention。

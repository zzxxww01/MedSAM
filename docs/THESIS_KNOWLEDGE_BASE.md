...
| HD95 ↓ | 4.8305 | **2.2511** | 2.9145 | -39.7% |
| ASD ↓ | 0.5378 | **0.2463** | 0.5389 | — |
...
| 可训练 | LoRA 旁路 (73,728参数) + Mask Decoder |
...
|------|------|-----------------|------|
| DSC | 0.9596 | **0.9620** | +0.25% (突破0.96大关) |
| HD95 | **2.2511** | 2.9145 | +0.66 (可能受少数outlier影响) |
| ASD | **0.2463** | 0.5389 | +0.29 |

> DSC 创历史新高。HD95/ASD 略逊于 A3R3，分析见下文。

### 6.7 C3 的 HD95/ASD 分析

- HD95 取第95百分位最大边界距离，40例中仅需1-2例 outlier 即可拉高
- LG-Adapter 的膨胀卷积可能在个别异常解剖病例上产生过强高频响应
- DSC 作为全局分割核心指标优先级最高，C3 在 DSC 上的绝对领先更有意义
- HD95/ASD 的轻微劣化可通过后处理（形态学平滑）改善
...
| **C3** | **LG-Adapter + A3R3** | **0.9620** | **2.9145** | **0.5389** | **全局最优 DSC** |
...
HD95 对 outlier 极敏感（40例中1-2例即可拉高），可能是膨胀卷积在极少数异常病例上的过强响应。DSC 作为全局核心指标更有意义，C3 在 DSC 上创历史最优(0.962)。HD95/ASD 可通过形态学后处理改善。

### Q6：LoRA 在 NLP 很成功，为什么在这里失败？

NLP 任务（文本分类等）对特征空间的重塑需求远小于医学图像的密集像素预测。FLARE22 包含 13 类形态差异巨大的器官，需要 Encoder 进行深层非线性空间重塑。LoRA 的低秩旁路（仅73K参数，占0.08%）表达能力不足以跨越自然图像→医学CT的语义鸿沟。
